...
const Pokemon = () => {
  ...
  if (!pokemonUrls || (pokemonUrls as string[]).length === 0)
  ...
  const displayedPokemonUrls = Array.isArray(pokemonUrls)
    ? showOnlyFirstFive
      ? pokemonUrls.slice(0, 5)
      : pokemonUrls
    : [];
  ...
  return (
    <>
      ...
      >
        ...
        <button onClick={() => setIsAllShiny(!isAllShiny)}>
          Toggle All Shiny
        </button>
      </div>
      <div style={{ paddingTop: "50px" }}>
        <div className="pokemon-grid">
          {transposeArray(chunkArray(displayedPokemonUrls, 5)).map(
            (chunk, chunkIndex) => (
